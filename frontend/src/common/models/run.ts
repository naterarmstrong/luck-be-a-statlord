...
export class RunDetails {
  ...
  recordSymbol(rentsPaid: number, symbol: Symbol, value: number) {
    ...
    if (!this.inventory!.get(symbol) && symbol !== Symbol.Empty) {
      console.error(
        "Encountered symbol I shouldn't have",
        symbol,
        this.inventory,
      );
    }
  ...
  recordSymbolDestroyed(rentsPaid: number, symbol: Symbol) {
    ...
    if (this.inventory!.get(symbol)! < 0) {
      console.error(
        "Encountered symbol with negative count",
        symbol,
        this.inventory,
      );
    }
  ...
  getBestNSymbols(n: number): Array<Symbol> {
    const ret = new Array(...this.symbolDetails.entries())
...
export function newEarnedValue(
  coins: number,
  rerolls: number,
  removals: number,
  essences: number,

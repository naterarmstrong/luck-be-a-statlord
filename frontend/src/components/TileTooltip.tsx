...
const StyledTooltip = styled(
  ({ item, className, ...props }: StyledTooltipProps) => (
...
const CoinVal: React.FC<CoinValProps> = ({ coins, omitGives }) => {
  return (
    <Typography
      style={{ color: rarityColor(Rarity.Rare) }}
      sx={{ fontSize: `40px`, lineHeight: 0.7 }}
      display="inline"
    >
      {omitGives ? null : <ColoredText color="white">Gives </ColoredText>}{" "}
      <SymImg
        tile={Symbol.Coin}
        size={30}
        omitTooltip
...
const TileTooltip: React.FC<TileTooltipProps> = ({ tile, children }) => {
  return (
    <StyledTooltip title={getTileTooltipContents(tile)} item={!isSymbol(tile)}>
      {children}
    </StyledTooltip>
  );
};
...
export function getTileTooltipContents(tile: Symbol | Item) {
  ...
  if (description) {
    ...
    for (const piece of descSplit) {
      if (lastWasStr) {
        ...
        if (piece.startsWith("coin") && piece.length !== 4) {
          ...
          descPieces.push(
            <SymImg omitTooltip textAlign size={30} tile={tile as Symbol} />,
          );
          ...
          descPieces.push(
            <SymImg
              ...
              textAlign
              size={30}
              tile={IIDToSymbol(piece)}
            />,
          );
        } else if (IIDToItem(piece) !== Item.ItemMissing) {
          descPieces.push(
            <SymImg omitTooltip textAlign size={30} tile={IIDToItem(piece)} />,
          );
        } else if (isGroup(piece)) {
          const symbols = GROUP_MEMBERS[piece as Group];
          const symElements = symbols.map((s) => (
            <SymImg
              omitTooltip
              textAlign
              size={30}
              tile={s}
              style={{ marginLeft: 0.3, marginRight: 0.3 }}
            />
          ));
          ...
          descPieces.push(
            <SymImg omitTooltip textAlign size={30} tile={piece} />,
          );
          ...
          descPieces.push(
            <SymImg omitTooltip textAlign size={30} tile={token} />,
          );
          descPieces.push(
            <ColoredText color={TOKEN_COLOR[token]}>{numeric}</ColoredText>,
          );
        } else {
    ...
    descriptionElements = (
      <React.Fragment>
        ...
        />
        <Typography fontSize="40px" lineHeight={0.7} key="description">
          {descPieces}
        </Typography>
      </React.Fragment>
    );
  ...
  return (
    <React.Fragment>
      <Typography fontSize="50px" lineHeight={0.7} key="title">
        {" "}
        {getTitle(tile)} <SymImg tile={tile} omitTooltip textAlign />
      </Typography>
...
function processDescriptionText(text: string): React.ReactElement<any, any> {
  ...
  for (const word of text.split(" ")) {
    ...
    if (currentStr.endsWith("Very ") && word === "Rare") {
      ...
      retSoFar.push(
        <React.Fragment>
          <ColoredText color={rarityColor(Rarity.VeryRare)}>
            {"Very Rare "}
          </ColoredText>
        </React.Fragment>,
      );
      ...
      retSoFar.push(
        <React.Fragment>
          <ColoredText color={rarityColor(Rarity.Essence)}>
            {"each other "}
          </ColoredText>
        </React.Fragment>,
      );
    ...
    const needsHighlight =
      ...
      (!isNaN(Number(lowered.slice(0, -1))) &&
        ["x", "%"].includes(lowered.slice(-1))) ||
      (isStringRarity(word.slice(0, -1)) &&
        [",", "."].includes(lowered.slice(-1))) ||
      isStringRarity(word);
    if (needsHighlight) {
      if (currentStr !== "") {
      ...
      retSoFar.push(
        <React.Fragment>
          <ColoredText color={color}>{displayWord}</ColoredText>
          {extra}
        </React.Fragment>,
      );
    } else {

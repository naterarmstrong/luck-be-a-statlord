...
const DisplayRuns: React.FC<DisplayRunsProps> = ({
  runs,
  omitDuration,
  unlimitedHeight,
}) => {
  ...
  return (
    <Grid item sx={sx}>
      <Table>
        <TableHead>
          <TableRow>
            {runs.at(0)?.authorInfo !== undefined ? (
              <TableCell sx={{ maxWidth: 150 }}>User</TableCell>
            ) : null}
          ...
          {runs.map((run: RunInfo, index) => (
            <TableRow role="checkbox" key={run.number} selected={run.victory}>
              {run.authorInfo !== undefined ? (
                <TableCell>{run.authorInfo.username}</TableCell>
              ) : null}
              <TableCell>
                <Link
                  href={`/user/${run.authorInfo !== undefined ? run.authorInfo.userId : userId}/run/${run.number}`}
                >
                  {run.number}
                </Link>
              </TableCell>
              <TableCell>
                <Grid>
                  <Box
                    component="img"
                    style={{ width: "50px" }}
                    src={run.victory ? String(confirm) : String(dud)}
                  />
                  ...
                  {run.guillotine ? (
                    <Box
                      ...
                      src={
                        ITEM_TO_IMG.get(
                          Item.Guillotine,
                        ) /* TODO: Change this to guillotine when I add item images. */
                      }
                    />
                  ) : null}
                </Grid>

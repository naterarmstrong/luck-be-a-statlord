...
const columns: GridColDef[] = [
    ...
    renderCell: (params: any) => (
      <Box>
        <Box
          component="img"
          style={{ width: "50px" }}
          src={ITEM_TO_IMG.get(params.value)}
        />
  ...
  {
    field: "destroy_rate",
    headerName: "Destroy Rate",
    minWidth: 150,
    valueGetter: (params: GridValueGetterParams) =>
      (
        (100 * params.row.total_destroyed_games) /
        params.row.total_games
      ).toFixed(2),
  },
  {
    field: "destroy_win_rate",
    headerName: "Destroy Win Rate",
    minWidth: 150,
    valueGetter: (params: GridValueGetterParams) =>
      (
        (100 * params.row.destroyed_won_games) /
        (params.row.total_destroyed_games === 0
          ? 1
          : params.row.total_destroyed_games)
      ).toFixed(2),
  },

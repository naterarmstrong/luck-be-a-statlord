...
const EaterCalculator: React.FC = () => {
  ...
  const [item, setItem] = useState<AdjacencyItem | Item.ItemMissing>(
    Item.ItemMissing,
  );
  ...
  const calculate = () => {
    ...
    const bothNotPresent =
      ((eTotal - 20) / eTotal) * ((eTotal - 1 - 20) / (eTotal - 1));
    ...
    const avgShows = avgSpins * (bothPresent + oneIsPresent * 0.5);
    const breakevenAt1 = avgShows / (20 / Math.max(eTotal - 1, 20)) + avgSpins;
    const breakevenAt2 =
      (2 * avgShows) / (20 / Math.max(eTotal - 1, 20)) + avgSpins;
    const breakevenAt3 =
      (3 * avgShows) / (20 / Math.max(eTotal - 1, 20)) + avgSpins;
      ...
      <Grid item xs={2}>
        <FormControl sx={{ minWidth: 280 }}>
          ...
          <Select
            ...
            autoWidth
            onChange={(e) =>
              setItem(e.target.value as AdjacencyItem | Item.ItemMissing)
            }
          >
            <MenuItem value={Item.ItemMissing}>None</MenuItem>
            <MenuItem value={Item.Protractor}>
              <SymImg tile={Item.Protractor} textAlign />
              Protractor
            </MenuItem>
            <MenuItem value={Item.ProtractorEssence}>
              <SymImg tile={Item.ProtractorEssence} textAlign />
              Protractor Essence
            </MenuItem>
            <MenuItem value={Item.Telescope}>
              <SymImg tile={Item.Telescope} textAlign />
              Telescope
            </MenuItem>
            <MenuItem value={Item.TelescopeEssence}>
              <SymImg tile={Item.TelescopeEssence} textAlign />
              Telescope Essence
            </MenuItem>
          </Select>
        </FormControl>
      ...
      <Grid item xs={6}>
        {stats !== undefined ? (
          <React.Fragment>
            ...
            <Tooltip
              title={
                "The new symbol is worth 1 less than the average symbol on your board."
              }
            >
              <Typography variant="h5">
                At -1
                <SymImg tile={Symbol.Coin} omitTooltip textAlign />:{" "}
                {stats.breakevenAt1.toFixed(1)} spins to break even
              </Typography>
            </Tooltip>
            <Tooltip
              title={
                "The new symbol is worth 2 less than the average symbol on your board."
              }
            >
              <Typography variant="h5">
                At -2
                <SymImg tile={Symbol.Coin} omitTooltip textAlign />:{" "}
                {stats.breakevenAt2.toFixed(1)} spins to break even
              </Typography>
            </Tooltip>
            <Tooltip
              title={
                "The new symbol is worth 3 less than the average symbol on your board."
              }
            >
              <Typography variant="h5">
                At -3
                <SymImg tile={Symbol.Coin} omitTooltip textAlign />:{" "}
                {stats.breakevenAt3.toFixed(1)} spins to break even
              </Typography>
            </Tooltip>
          </React.Fragment>
        ) : null}

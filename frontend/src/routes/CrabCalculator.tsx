...
const CrabCalculator: React.FC = () => {
  ...
  const [lineOdds, setLineOdds] = useState<Array<number> | undefined>(
    undefined,
  );
  const [spinOdds, setSpinOdds] = useState<Array<number> | undefined>(
    undefined,
  );
  ...
  const crabDisplay = (nCrabs: number, odds: number): JSX.Element => {
    return (
      <Typography variant="h5">
        {(100 * odds).toFixed(2)}% Chance for{" "}
        {numberToRange(nCrabs).map(() => (
          <SymImg tile={Symbol.Crab} style={{ marginBottom: -8 }} />
        ))}{" "}
        {nCrabs < 5 ? " or more" : null}
      </Typography>
    );
  };
  ...
  return (
    <Grid container alignItems="center" justifyContent="center">
      ...
      <Grid item xs={12}>
        <Typography variant="h6">Find coins from having crabs</Typography>
      </Grid>
      <Grid item xs={2} />
      ...
      <Grid item xs={3} />
      <Grid item xs={6}>
        {lineOdds !== undefined && spinOdds !== undefined ? (
          <React.Fragment>
            {numberToRange(calcCrabs).map((n) =>
              crabDisplay(
                n,
                spinOdds.slice(n).reduce((a, b) => a + b),
              ),
            )}
            <Typography variant="h3">
              {coinTotal!.toFixed(2)}

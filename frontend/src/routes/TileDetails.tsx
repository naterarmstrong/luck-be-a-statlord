...
import {
  getRarity,
  getTileTooltipContents,
  symbolHighColor,
} from "../components/TileTooltip";
...
const TileDetails: React.FC<TileDetailsProps> = ({ mode }) => {
  ...
  if (
    ...
    isSymbol(searchParams.get("symbol") as any)
  ) {
  ...
  if (
    ...
    isItem(searchParams.get("item") as any)
  ) {
    startTile = searchParams.get("item") as Item;
  }
  if (
    mode === "essence" &&
    searchParams.get("essence") !== null &&
    isItem(searchParams.get("essence") as any)
  ) {
    startTile = searchParams.get("essence") as Item;
  }
  if (
    searchParams.get("secondarySymbol") !== null &&
    isItem(searchParams.get("secondarySymbol") as any)
  ) {
    secondaryStartItem = searchParams.get("secondaryItem") as Item;
  }

  const [tile, setTile] = useState<Symbol | Item>(startTile);
  const [secondarySymbol, setSecondarySymbol] =
    useState<Symbol>(secondaryStartSymbol);
  const [secondaryItem, setSecondaryItem] = useState<Item>(secondaryStartItem);
  const [symbolPairPerf, setSymbolPairPerf] = useState<PairPerformance | null>(
    null,
  );
  const [itemPairPerf, setItemPairPerf] = useState<PairPerformance | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const fetchPairStats = async () => {
      const response = await fetch(
        `${API_ENDPOINT}/tilePairs/${tile}/with/${secondarySymbol}`,
      );
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    const fetchSymbolStats = async () => {
      const response = await fetch(
        `${API_ENDPOINT}/tilePairs/${tile}/with/${secondaryItem}`,
      );
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    for (const stats of fullStats) {
      ...
      if (
        stats.chosen_won_games / stats.chosen_games >
        myStats.chosen_won_games / myStats.chosen_games
      ) {
        if (stats.rarity === myStats.rarity) {
          relativePWrRank += 1;
        }
    ...
    const roundedPickedWinrate =
      Math.round((10000 * myStats.chosen_won_games) / myStats.chosen_games) /
      100;
  ...
  return (
    <Grid container alignItems="center" justifyContent="center">
      ...
      <Grid item xs={8}>
        {mode === "symbol" && (
          <SymbolSelector symbol={tile as Symbol} setSymbol={setTile} />
        )}
        {mode === "item" && (
          <ItemSelector item={tile as Item} setItem={setTile} />
        )}
        {mode === "essence" && (
          <ItemSelector item={tile as Item} setItem={setTile} essence />
        )}
      </Grid>
      <Grid item xs={4} alignSelf="start">
        ...
        >
          <CardContent>{getTileTooltipContents(tile)}</CardContent>
        </Card>
        {fullStats.length > 0 &&
        ratings &&
        pickedAndTotalSufficientlyDifferent(
          ratings.PickedGames,
          ratings.Games,
        ) ? (
          <Card sx={{ mb: 1, ml: 3, mr: 3 }}>
            <CardContent sx={{ mb: -2 }}>
              ...
              <Typography variant="h3" lineHeight={0.5}>
              ...
              <Box justifyContent={"center"}>
                <Typography sx={{ display: "inline-block", mx: "50px" }}>
                  ...
                  <Typography
                    color={rarityColor(getRarity(tile))}
      ...
      <Grid item xs={8} alignSelf="start">
        <Grid container>
          <Grid item xs={4}>
            <Typography variant="h6">With symbols:</Typography>
          </Grid>
          <Grid item xs={8}>
            <SymbolSelector
          ...
          <Grid item xs={8}>
            {symbolPairPerf !== null ? (
              <Grid container>
                <Grid item>
                  <SymImg tile={tile} /> <SymImg tile={secondarySymbol} />:{" "}
                  {symbolPairPerf.WinrateTogether}% win rate.{" "}
                  {(
                    (100 * symbolPairPerf.GamesTogether) /
                    symbolPairPerf.TotalTotalGames
                  ).toFixed(1)}
                  % of games. <br />
                </Grid>
                <Grid item>
                  <SymImg tile={tile} /> <SymImg tile={Symbol.Dud} />:{" "}
                  {symbolPairPerf.WinrateTile1}% win rate.{" "}
                  {(
                    (100 * symbolPairPerf.GamesApartTile1) /
                    symbolPairPerf.TotalTotalGames
                  ).toFixed(1)}
                  % of games. <br />
                </Grid>
                <Grid item>
                  <SymImg tile={Symbol.Dud} /> <SymImg tile={secondarySymbol} />
                  : {symbolPairPerf.WinrateTile2}% win rate.{" "}
                  {(
                    (100 * symbolPairPerf.GamesApartTile2) /
                    symbolPairPerf.TotalTotalGames
                  ).toFixed(1)}
                  % of games. <br />
                </Grid>
          ...
          <Grid item xs={4}>
            <Typography variant="h6">With items:</Typography>
          </Grid>
          <Grid item xs={8}>
          ...
          <Grid item xs={8}>
            {itemPairPerf !== null ? (
              <Grid container>
                <Grid item>
                  <SymImg tile={tile} /> <SymImg tile={secondaryItem} />:{" "}
                  {itemPairPerf.WinrateTogether}% win rate.{" "}
                  {(
                    (100 * itemPairPerf.GamesTogether) /
                    itemPairPerf.TotalTotalGames
                  ).toFixed(1)}
                  % of games. <br />
                </Grid>
                <Grid item>
                  <SymImg tile={tile} /> <SymImg tile={Symbol.Dud} />:{" "}
                  {itemPairPerf.WinrateTile1}% win rate.{" "}
                  {(
                    (100 * itemPairPerf.GamesApartTile1) /
                    itemPairPerf.TotalTotalGames
                  ).toFixed(1)}
                  % of games. <br />
                </Grid>
                <Grid item>
                  <SymImg tile={Symbol.Dud} /> <SymImg tile={secondaryItem} />:{" "}
                  {itemPairPerf.WinrateTile2}% win rate.{" "}
                  {(
                    (100 * itemPairPerf.GamesApartTile2) /
                    itemPairPerf.TotalTotalGames
                  ).toFixed(1)}
                  % of games. <br />
                </Grid>
...
function pickedAndTotalSufficientlyDifferent(
  picked: number,
  total: number,
): boolean {

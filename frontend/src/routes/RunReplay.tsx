...
const RunReplay: React.FC = () => {
  ...
  useEffect(() => {
    if (!isNaN(Number(searchParams.get("spinIdx")))) {
      updateDisplayOptions({ setSpinIdx: Number(searchParams.get("spinIdx")) });
    }
  }, [runInfo]);
    ...
    const fetchRunData = async () => {
      ...
      jsonData.SymbolDetails.forEach((s: any) => {
        details.symbolDetails.set(s.symbol, {
          ...
          coinsByRentPayment: [],
          ...(s.earliestRentAdded !== null && {
            earliestRentAdded: s.earliestRentAdded,
          }),
        });
      });
  ...
  const updateDisplayOptions = (e: UpdateDisplayOptionsArgs) => {
    ...
    setSearchParams({ spinIdx: `${newOptions.spinIdx}` });
  };
  ...
  if (!runInfo || !runInfo.details) {
    return <Typography variant="h3">Loading Run...</Typography>;
  }
  ...
  return (
    <Box>
        ...
        >
          <Grid item xs={12}>
            <Typography variant="h4">
              {UID !== undefined ? (
                <Button
                  onClick={() => {
                    navigate(`/user/${UID}/run/${runInfo.number - 1}`);
          ...
          <Grid item xs={12}>
            <Grid container spacing={1}>
              <Grid item>
                <Button
                  onClick={() => updateDisplayOptions({ setSpinIdx: 0 })}
                  variant="contained"
                >
                  First
                </Button>
              ...
              <Grid item>
                <Button
                  onClick={() => updateDisplayOptions({ decSpinIdx: true })}
                  variant="contained"
                >
                  Previous
                </Button>
              ...
              <Grid item>
                <Typography variant="h6">
                  Spin{" "}
                  {runInfo.details.spins[displayOptions.spinIdx].number + 1} of{" "}
                  {runInfo.spins + 1}
                </Typography>
              </Grid>
              <Grid item>
                <Button
                  onClick={() => updateDisplayOptions({ incSpinIdx: true })}
                  variant="contained"
                >
                  Next
                </Button>
          ...
          <Grid item xs={12}>
            <Grid container spacing={5} justifyContent="space-around">
              ...
              <Grid item minWidth={`${8 * 76}px`}>
                <GameBoard
                  pxSize={8}
                  symbols={getSpinSymbols()}
                  coins={
                    runInfo.details.spins[displayOptions.spinIdx].symbolValues
                  }
                  displayCoins={displayOptions.displayCoins}
                />
              </Grid>
              <Grid item>
          ...
          <Grid
            ...
            alignSelf="start"
            marginLeft="50px"
            marginTop={displayOptions.postEffects ? "-100px" : "-110px"}
          >
            {getCoinDisplay(
              runInfo,
              displayOptions.postEffects,
          ...
          <Grid container justifyContent="center" spacing={3}>
            <Grid item>
              <Button
                onClick={() => updateDisplayOptions({ toggleEffects: true })}
                variant="contained"
              >
                Toggle effects (e)
              </Button>
            ...
            <Grid item>
              <Button
                onClick={() => updateDisplayOptions({ toggleCoins: true })}
                variant="contained"
              >
                Toggle Coins (c)
              </Button>
      ...
      <Typography variant="h2">Run Stats</Typography>
      <Grid
        container
        justifyContent="center"
        alignItems="center"
      ...
      >
        <Grid item>
          <Typography variant="h4" color={runInfo.victory ? "green" : "red"}>
            {runInfo.victory
              ? "Victory"
              : `Defeat after ${runInfo.spins} spins`}
          </Typography>
          {runInfo.guillotine ? (
...
const getCoinDisplay = (
  ...
  const dueDisplay = (
    ...
    >
      {diffDisplay}
      {dueInfo.due}{" "}
      {dueInfo.spinsRemaining > 0 ? `in ${dueInfo.spinsRemaining}` : "now"}
    </Typography>
  );
...
const getFirstItemDisplay = (spin: SpinData, postEffects: boolean) => {
  ...
  return (
    <Grid container spacing={1} justifySelf="end" maxWidth={12 * 6 * 4}>
      {items.map((i) => (
        <Grid item xs={4}>

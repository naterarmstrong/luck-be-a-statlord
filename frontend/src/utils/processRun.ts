...
export function processRun(text: string): RunInfo {
  ...
  for (const [idx, spinText] of spins.slice(1).entries()) {
    ...
    if (
      spinData.postEffectItems.find(
        (v) => v.item === Item.GuillotineEssence,
      ) !== undefined &&
      spinData.coinTotal > 1000000000
    ) {
      isGuillotine = true;
    }
    ...
    spinData.symbolsAddedChoice.forEach((symbol: Symbol) =>
      details.recordSymbolAdded(curRentsPaid, symbol, true),
    );
    spinData.symbolsAddedNoChoice.forEach((symbol: Symbol) =>
      details.recordSymbolAdded(curRentsPaid, symbol, false),
    );
    spinData.symbolsDestroyed.forEach((symbol: Symbol) =>
      details.recordSymbolDestroyed(curRentsPaid, symbol),
    );
    spinData.symbolsRemoved.forEach((symbol: Symbol) =>
      details.recordSymbolRemoved(curRentsPaid, symbol),
    );
    spinData.symbolsTransformed.forEach((symbol: Symbol) =>
      details.recordSymbolRemoved(curRentsPaid, symbol),
    );

    spinData.itemsAddedChoice.forEach((item: Item) =>
      details.recordItemAdded(curRentsPaid, item, true),
    );
    spinData.itemsAddedNoChoice.forEach((item: Item) =>
      details.recordItemAdded(curRentsPaid, item, false),
    );
    spinData.itemsDestroyed.forEach((item: Item) =>
      details.recordItemDestroyed(curRentsPaid, item),
    );
    ...
    for (let i = 0; i < 20; i++) {
      ...
      details.recordSymbol(
        rentsPaid,
        spinData.preEffectLayout[i].symbol,
        spinData.symbolValues[i].coins,
      );
...
function couldBeFloor20(spinData: SpinData): boolean {
  if (spinData.number === 0) {
    const dudCount = spinData.preEffectLayout
      .map((ss) => ss.symbol)
      .filter((s) => s === Symbol.Dud).length;
    if (dudCount !== 3) {
      return false;
    }
  }

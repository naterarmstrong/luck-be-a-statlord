  ...
  try {
        ...
        if (effect.isSymbolDestroyed()) {
          if (
            ...
            (!effects[effects.length - 1].equals(effect) &&
              !(
                effects.length >= 2 &&
                effects[effects.length - 2].equals(effect)
              ))
          ) {
    ...
    while (!sp.done()) {
      if (sp.isVictory()) {
        ...
        if (
          symbols.length === 1 &&
          symbols[0] === Symbol.HexOfDestruction &&
          spinNum >= 90
        ) {
        ...
        sp.isPostEffectSymbols() &&
        postEffectItems.some(
          (si: SpinItem) =>
            si.item === Item.Coffee || si.item === Item.CoffeeEssence,
        )
      ) {
    ...
    if (!sp.done()) {
      console.error(
        `Lines left!! ${sp.lines.length} total, ${sp.lineIdx} index`,
      );
      sp.markEnd(true);
    } else {
      sp.markEnd(sawError);
...
class SpinParser {
  ...
  getSpinNumber(): number {
    ...
    if (isNaN(spinNum)) {
      this.sawError = true;
      throw new Error(
        `Failed to detect spin number from text: ${this.lines[this.lineIdx - 1]}`,
      );
    }
    if (this.isTrace()) {
  ...
  getCoinsBefore(): number {
    const coinsBeforeMatch = this.readLine().match(
      "Currently have ([0-9]*) coins",
    )?.[1];
  ...
  getPreEffectSymbols(): Array<SpinSymbol> {
    ...
    if (this.isError()) {
      const spinLayoutMatch = headerLine.match(
        "Spin layout before effects is:",
      );
      if (!spinLayoutMatch) {
        this.sawError = true;
        console.error(
          `Could not find pre effects layout heading on line.`,
          headerLine,
        );
      }
    }
  ...
  getPreEffectItems(): Array<SpinItem> {
    ...
    if (this.isError()) {
      ...
      if (!itemHeaderMatch) {
        this.sawError = true;
        console.error(
          `Could not find pre effects items heading on line.`,
          headerLine,
        );
      }
    }

    const itemTexts = this.readLine()
      .split("[")[1]
      .split("]")[0]
      .split(",")
      .map((s) => s.trim());
  ...
  getPostEffectSymbols(): Array<SpinSymbol> {
    ...
    if (this.isError()) {
      if (!headerLine.startsWith("Spin layout after effects is:")) {
        this.sawError = true;
        console.error(
          `Could not find post effects layout heading on line.`,
          headerLine,
        );
      }
    }
  ...
  getPostEffectItems(): Array<SpinItem> {
    ...
    if (this.isError()) {
      ...
      if (!itemHeaderMatch) {
        console.error(
          `Could not find post effects items heading on line.`,
          headerLine,
        );
        this.sawError = true;
      }
  ...
  getPostEffectItemValues(): Array<EarnedValue> {
    const headerLine = this.readLine();
    if (this.isError()) {
      const itemHeaderMatch = headerLine.match(
        "Item values after effects are:",
      );
      if (!itemHeaderMatch) {
        console.error(
          `Could not find post effects items heading on line.`,
    ...
    const valueTexts = this.readLine()
      .split("[")[1]
      .split("]")[0]
      .split(",")
      .map((s) => s.trim());
  ...
  getDestroyedSymbols(): Array<Symbol> {
    ...
    if (this.isError()) {
      const itemHeaderMatch = headerLine.match(
        "There are [0-9]+ destroyed symbols:",
      );
      if (!itemHeaderMatch) {
        console.error(
          `Could not find symbols destroyed heading on line.`,
          headerLine,
        );
        this.sawError = true;
      }
    ...
    const symbolTexts = this.readLine()
      .split("[")[1]
      .split("]")[0]
      .split(",")
      .map((s) => s.trim());
  ...
  getDestroyedItems(): Array<Item> {
    ...
    if (this.isError()) {
      const itemHeaderMatch = headerLine.match(
        "There are [0-9]+ destroyed items:",
      );
      if (!itemHeaderMatch) {
        console.error(
          `Could not find items destroyed heading on line.`,
          headerLine,
        );
        this.sawError = true;
      }
  ...
  isItemDestroyed(): boolean {
    return (
      this.peek().startsWith("Destroyed item") ||
      this.peek().startsWith("Manually destroyed item")
    );
  }
  ...
  getItemDestroyed(): Item {
    ...
    const item = line.startsWith("Destroyed item")
      ? IIDToItem(line.split(" ")[3].split(",")[0])
      : IIDToItem(line.split(" ")[4]);
  ...
  getItemDestroyCounter(): Item {
    ...
    if (!itemMatch || IIDToItem(itemMatch[1]) === Item.ItemMissing) {
      throw new Error(
        `Failed to match item when destroy counter incremented: ${line}`,
      );
    } else if (this.isTrace()) {
  ...
  getGainedCoins(): number {
    const line = this.readLine();
    const gainedCoinsMatch = line.match(
      "Gained (-?[0-9]+|inf) coins this spin",
    );
    if (
      !gainedCoinsMatch ||
      gainedCoinsMatch.length !== 2 ||
      (isNaN(Number(gainedCoinsMatch[1])) && gainedCoinsMatch[1] !== "inf")
    ) {
    ...
    return gainedCoinsMatch[1] === "inf"
      ? Number.POSITIVE_INFINITY
      : Number(gainedCoinsMatch[1]);
  }

  getCoinTotal(): number {
    const line = this.readLine();
    const coinTotalMatch = line.match(
      "Coin total is now (-?[0-9]+|inf) after spinning",
    );

    if (
      !coinTotalMatch ||
      coinTotalMatch.length !== 2 ||
      (isNaN(Number(coinTotalMatch[1])) && coinTotalMatch[1] !== "inf")
    ) {
      throw new Error(`Could not read gained coins in ${line}`);
    }
    ...
    return coinTotalMatch[1] === "inf"
      ? Number.POSITIVE_INFINITY
      : Number(coinTotalMatch[1]);
  }
  ...
  isItemAdded(): boolean {
    return (
      this.peek().startsWith("Added item") ||
      this.peek().startsWith("Skipped items")
    );
  }
  ...
  parseEffectObject(text: string): any {
    ...
    const fix1 = text.replace(
      /\b:(Pop-up|Reels|Item|Slot Icon):\[[^,^}]*\]/gm,
      ':"$1"',
    );
    ...
    const fix7 = fix6
      .replace(/\[(\w+), (\w+)\]/gm, '["$1", "$2"]')
      .replace(/\[(\w+), (\w+), (\w+)\]/gm, '["$1", "$2", "$3"]')
      .replace(
        /\[(\w+), (\w+), (\w+), (\w+), (\w+), (\w+)\]/gm,
        '["$1", "$2", "$3", "$4", "$5", "$6"]',
      )
      .replace(
        /\[(\w+), (\w+), (\w+), (\w+), (\w+), (\w+), (\w+), (\w+), (\w+)\]/gm,
        '["$1", "$2", "$3", "$4", "$5", "$6", "$7", "$8", "$9"]',
      );
    const fix8 = fix7.replace(
      /"tiles_to_add":\[\w+, \w+, \w+, \w+, \w+, \w+, \w+, (.*)\]/gm,
      '"tiles_to_add": "many"',
    );
  ...
  parseValue(valueText: string): EarnedValue {
    ...
    const valueTextMatches = valueText.match(
      /(-?[0-9]+|inf)(e[0-9]+)?(v[0-9]+)?(r[0-9]+)?/,
    );
    if (
      !valueTextMatches ||
      valueTextMatches.length !== 5 ||
      valueTextMatches[0] === undefined
    ) {
    ...
    let coins =
      valueTextMatches[1] === "inf"
        ? Number.POSITIVE_INFINITY
        : Number(valueTextMatches[1]);
  ...
  isError(): boolean {
    return (
      this.debug === DebugLevel.Error ||
      this.debug === DebugLevel.Trace ||
      this.debug === DebugLevel.Custom
    );
  }

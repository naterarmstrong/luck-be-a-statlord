import { SessionDatabase, Session } from '../db/sessionDb';
import { LiveSessionData, LiveSessionState } from '../models/liveSessionData';


export class SessionController {
    private db: SessionDatabase;

    constructor() {
        // todo: create real db
        this.db = new SessionDatabase();
    }

    createSession(session: Session) {
        this.db.createSession(session);
    }

    getSession(id: string): Session | undefined {
        return this.db.getSession(id)
    }

    createLiveSession(sessionId: string) {
        const oldSession = this.db.getSession(sessionId);
        if (!oldSession) {
            // TODO: bubble up error
            return;
        }
        const liveSessionData = new LiveSessionData(false, 0, oldSession.videoUrl, Date.now() / 1000);
        return this.db.createLiveSession(sessionId, liveSessionData);
    }

    // We extrapolate the current video time out from the last playing event received
    getLiveSession(sessionId: string): LiveSessionState | undefined {
        const liveSessionData = this.db.getLiveSession(sessionId);
        if (!liveSessionData || !liveSessionData.isPlaying) {
            return liveSessionData?.toState();
        }
        const timePassed = (Date.now() / 1000) - liveSessionData.timestampSeconds;
        const newState = {
            isPlaying: true,
            videoSeconds: timePassed + liveSessionData.videoSeconds,
            videoUrl: liveSessionData.videoUrl
        };
        return newState;
    }

    updateLiveSessionWithURL(sessionId: string, state: LiveSessionState) {
        const data = new LiveSessionData(state.isPlaying, state.videoSeconds, state.videoUrl, Date.now() / 1000);
        return this.db.updateLiveSession(sessionId, data);
    }

    // Update a live session, but do not update the URL
    updateLiveSession(sessionId: string, state: LiveSessionState) {
        const oldData = this.db.getLiveSession(sessionId);
        if (!oldData) {
            console.log("ERROR: Failed to find live session data");
            return;
        } else if (oldData.videoUrl != state.videoUrl) {
            console.log("Ignoring update due to outdated video URL");
            return;
        }
        const data = new LiveSessionData(state.isPlaying, state.videoSeconds, oldData.videoUrl, Date.now() / 1000);
        return this.db.updateLiveSession(sessionId, data);
    }
}
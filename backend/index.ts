...
import {
  RUN_PROCESSING_VERSION,
  RunInfo,
  SpinData,
  SpinInfo,
} from "../frontend/src/common/models/run";
import {
  bestUsersQuery,
  essenceWinratesQuery,
  itemWinratesQuery,
  sequelize,
  symbolWinratesQuery,
  userStatsQuery,
...
app.post("/register", async (req: AuthorizedRequest, res) => {
  if (req.username && req.username !== req.body.username) {
    console.log(
      `Received registration request when user already logged in: username: ${req.username}`,
    );
  ...
  if (user) {
    let token = jwt.sign(
      { username: username, id: (user as any).id },
      JWT_SECRET,
      { expiresIn: DAY },
    );
...
app.post("/login", async (req: AuthorizedRequest, res) => {
  ...
  if (userInDB === null) {
    ...
    let token = jwt.sign(
      { username: username, id: (userInDB as any).id },
      JWT_SECRET,
      { expiresIn: DAY },
    );
...
app.post("/uploadRuns", async (req: AuthorizedRequest, res) => {
  ...
  for (const run of req.body as RunInfo[]) {
    try {
      ...
      for (const [symbol, details] of run.details!.symbolDetails.entries()) {
        ...
        for (let i = 0; i < 14; i++) {
          if (
            details.coinsByRentPayment[i] != 0 ||
            details.timesAddedChoiceByRent[i] != 0
          ) {
            symbolDetailsByRent.push({
              symbol,
              rent: i,
              value: details.coinsByRentPayment[i]
                ? details.coinsByRentPayment[i]
                : 0,
              timesAdded: details.timesAddedChoiceByRent[i]
                ? details.timesAddedChoiceByRent[i]
                : 0,
            });
          }
        }
      ...
      for (const [item, details] of run.details!.itemDetails.entries()) {
        ...
        for (let i = 0; i < 14; i++) {
          if (
            details.timesAddedByRent[i] != 0 ||
            details.timesDestroyedByRent[i] != 0
          ) {
            itemDetailsByRent.push({
              item,
              rent: i,
              timesDestroyed: details.timesDestroyedByRent[i]
                ? details.timesDestroyedByRent[i]
                : 0,
              timesAdded: details.timesAddedByRent[i]
                ? details.timesAddedByRent[i]
                : 0,
            });
          }
        }
  ...
  const sEnd = Date.now();
  console.log(
    `Time uploading ${req.body.length} runs: ${msToTime(sEnd - sStart)}`,
  );
...
app.get("/user/:id/runs", async (req, res) => {
  ...
  const runs = await sequelize.query(
    `SELECT * FROM Runs WHERE Runs.UserId = ${parseInt(req.params.id, 10)}`,
...
app.get("/user/:id/stats", async (req, res) => {
  ...
  const runs = await Run.findAll({
    attributes: ["number", "victory", "date"],
    where: {
      date: {
        [Op.lt]: Date.now(),
      },
...
app.get("/symbolStats", async (req, res) => {
  const stats = await sequelize.query(symbolWinratesQuery, {
    type: QueryTypes.SELECT,
    logging: false,
  });
...
app.get("/itemStats", async (req, res) => {
  const stats = await sequelize.query(itemWinratesQuery, {
    type: QueryTypes.SELECT,
    logging: false,
  });
...
app.get("/essenceStats", async (req, res) => {
  const stats = await sequelize.query(essenceWinratesQuery, {
    type: QueryTypes.SELECT,
    logging: false,
  });
...
app.get("/tilePairs/:tile1/with/:tile2", async (req, res) => {
  ...
  if (
    (!isSymbol(tile1) && !isItem(tile1)) ||
    (!isSymbol(tile2) && !isItem(tile2))
  ) {
...
function generateTilePairsQuery(
  tile1: Symbol | Item,
  tile2: Symbol | Item,
  apart: boolean,
  reverse: boolean,
